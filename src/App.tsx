...
export default function App() {
  ...
  const handleExportExcel = () => {
    ...
    const rows = feedbacks.map((f) => [
      ...
      f.isAnonymous ? 'Anonymous' : f.name.replace(/"/g, '""'),
      f.status,
      f.solution ? f.solution.content.replace(/"/g, '""') : 'None',
      f.solution ? f.solution.updatedBy : 'N/A',
      f.solution ? f.solution.updatedAt : 'N/A',
    ...
    link.setAttribute('download', 'dropbox_issues_and_suggestions_report.csv');
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);

    addToast('success', 'Submissions database successfully downloaded in CSV (Excel friendly) format!');
  };

  // PDF Export Trigger (Opens elegant formatted Print preview layout in new window)
  const handleExportPDF = () => {
    const printWindow = window.open('', '_blank');
    if (!printWindow) {
      addToast('error', 'Popup blocked! Please allow popups to compile the PDF print summary.');
      return;
    }

    const itemsHtml = feedbacks
      .map(
        (item) => `
      <div style="border-bottom: 1px solid #e2e8f0; padding-bottom: 24px; margin-bottom: 24px; page-break-inside: avoid;">
        <div style="display: flex; justify-content: space-between; font-size: 11px; color: #64748b; font-weight: 600; text-transform: uppercase;">
          <span>ID: ${item.id} | DATE: ${item.date}</span>
          <span style="color: #3b82f6;">${item.category} [${item.status}]</span>
        </div>
        <h3 style="margin-top: 8px; margin-bottom: 10px; font-size: 16px; font-weight: 700; color: #0f172a; font-family: sans-serif;">${item.subject}</h3>
        <p style="font-size: 13px; color: #334155; line-height: 1.6; margin-bottom: 12px; white-space: pre-line;">${item.description}</p>
        
        ${
          item.solution
            ? `
          <div style="background-color: #f0fdf4; border-left: 4px solid #10b981; padding: 14px; border-radius: 8px; margin-top: 12px; margin-bottom: 12px;">
            <h4 style="margin: 0 0 4px 0; font-size: 11px; color: #065f46; font-weight: bold; text-transform: uppercase;">Official Solution</h4>
            <p style="margin: 0; font-size: 12px; color: #047857; line-height: 1.5; font-weight: 500;">${item.solution.content}</p>
            <small style="font-size: 9px; color: #065f46; opacity: 0.8; display: block; margin-top: 6px;">Resolved by ${item.solution.updatedBy} on ${item.solution.updatedAt}</small>
          </div>
        `
            : ''
        }
        ...
        ${
          item.suggestions.length > 0
            ? `
          <div style="margin-top: 14px; padding-left: 12px; border-left: 2.5px solid #e2e8f0;">
            <h4 style="margin: 0 0 8px 0; font-size: 11px; color: #475569; font-weight: bold; text-transform: uppercase;">Discussion Suggestions (${item.suggestions.length})</h4>
            ${item.suggestions
              .map(
                (s) => `
              <div style="font-size: 11.5px; color: #475569; margin-bottom: 6px; line-height: 1.4;">
                <strong style="color: #1e293b;">${s.createdBy}</strong> <span style="font-size: 9px; color: #94a3b8;">(${s.createdAt})</span>: ${s.content}
              </div>
            `
              )
              .join('')}
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Feedback Dropbox Audit Report</title>
          <style>
            body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Arial, sans-serif; padding: 40px; color: #1e293b; max-width: 800px; margin: 0 auto; }
            h1 { font-size: 24px; font-weight: 800; border-bottom: 3px solid #3b82f6; padding-bottom: 10px; margin-bottom: 30px; display: flex; justify-content: space-between; align-items: flex-end; }
            .meta-header { display: flex; justify-content: space-between; font-size: 12px; color: #64748b; margin-bottom: 45px; background: #f8fafc; padding: 15px; border-radius: 8px; border: 1px solid #e2e8f0; }
            .footer { text-align: center; margin-top: 60px; font-size: 11px; color: #94a3b8; border-top: 1px solid #e2e8f0; padding-top: 15px; }
          </style>
        </head>
        <body>
          <h1>
            <span>Feedback Dropbox - Issue and Suggestion Audit</span>
            <span style="font-size: 12px; color: #64748b; font-weight: normal;">Issued: ${new Date().toLocaleDateString()}</span>
          </h1>
          <div class="meta-header">
            <div>
              <strong>Service:</strong> Dropbox Operations Center<br>
              <strong>Auditor Signature:</strong> Alex Polmond (HR Officer)<br>
              <strong>Data Scope:</strong> Enterprise Submissions
            </div>
            <div style="text-align: right;">
              <strong>Total Items:</strong> ${feedbacks.length}<br>
              <strong>Solved:</strong> ${feedbacks.filter((f) => f.status === 'Solved').length}<br>
              <strong>Pending:</strong> ${feedbacks.filter((f) => f.status === 'Pending').length}
            </div>
          </div>
          <div>
            ${itemsHtml}
          ...
          <div class="footer">
            Dropbox Issues & Suggestions system. All entries locally stored and synchronized. Exported format: PDF.
          </div>
          <script>
            setTimeout(function() {
              window.print();
    ...
    addToast('success', 'PDF compile success! Print dispatcher initialized.');
  };

  // Browser physical Print trigger
  const handlePrintReport = () => {
    addToast('info', 'Pre-configuring physical printing templates. Opening print screen...');
    setTimeout(() => {
      window.print();
    }, 400);
  };
